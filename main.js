...
function renderAdmin() {
  ...
  // 管理者レビューリクエスト
const reqList = JSON.parse(localStorage.getItem("adminRequests") || "[]");

area.innerHTML += `<h2 style="margin-top:30px;">Review Requests</h2>`;
...
reqList.forEach((r, i) => {
  ...
  area.innerHTML += `
    <div class="deck-card" style="border:1px solid var(--border);padding:10px;margin:10px 0;">
      <h3>${r.title} <span style="font-size:12px;">(${r.user})</span></h3>
      <div class="row">${top}</div>
      <div class="row">${bottom}</div>
      <button onclick="deleteRequest(${i})" class="del-btn">Delete</button>
    </div>
  `;
});
  ...
  decks.forEach((d, i) => {
    const top = d.cards.slice(0, 4).map(id => `
      <img src="${cardImg(id)}"
           style="width:var(--card-size);border-radius:8px;margin:3px;">
    `).join("");

    const bottom = d.cards.slice(4, 8).map(id => `
      <img src="${cardImg(id)}"
           style="width:var(--card-size);border-radius:8px;margin:3px;">
    `).join("");

    const avg = calcAvgElixir(d.cards);
    const isUser = i >= defaultDecks.length;
    ...
    area.innerHTML += `
      <div style="background:var(--card); border:1px solid var(--border);
        ...
        <div style="font-size:18px; font-weight:bold; margin-bottom:6px;
          ...
          <span>${i+1}. ${d.title} (${d.name || "NoName"})</span>
          <span class="elixir-box">
            <img src="cards/elixir.png" alt="elixir">
            <span class="elixir-num">${avg}</span>
          </span>
        </div>
        <div>${top}</div>
        <div>${bottom}</div>
          ...
          isUser
            ? `<button onclick="deleteDeck(${i})"
                style="margin-top:10px; padding:8px 14px;
                       background:#ff4444; border:none; border-radius:6px;
